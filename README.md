...
本任务的文件夹中包括所有文件数据，文件夹内具有ID的子文件夹代表一则消息信息，子文件夹内的json文件包括该消息的文本、用户、评论、转发信息，其他文件则为其附带的图片。
json文件内的字段说明如下：
...
除以上json文件外，本任务还为参赛选手们提供了汇总的info.xlsx文件，该文件存储了信源ID（已脱敏）、信源发布时间、信源文本内容以及信源包含的图片文件信息，字段说明如下：
| 字段 | 说明 |
| ----- | ----- |
|  new_id      |   脱敏后ID     |
|  created_at  |   信源发布时间 |
...
#### 提交格式
评测使用邮箱提交结果。参赛队伍需要将包含结果文件的邮件发送至邮箱liumingrui@uir.edu.cn，邮件的标题为“Competition-参赛队名”，邮件附件为任务的结果文件。任务一结果文件提交格式为csv文件，命名格式为：参赛队名_任务1.csv。
